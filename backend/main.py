...
class AudioClassifier:
    ...
    def inference(self, request:InferenceRequest):
        #production: fontend-> upload file to s3-> inference endpoint-> download from s3 bucket
        ...
        with torch.no_grad():
            ...
            probabilities = torch.softmax(output, dim=1) # dim=0 is batch, dim=1 is class (batch_size, num_class)
            ...
            for name, tensor in feature_maps.items():
                if tensor.dim() == 4: #(batch_size, channels, height, width)-> structure of feature maps
...
def main():
    audio_data, sample_rate = sf.read("1-100210-A-36.wav")
